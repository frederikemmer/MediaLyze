...
def validate_pattern_recognition_settings(settings: PatternRecognitionSettings) -> None:
    validate_regex_patterns(settings.show_season_patterns.series_folder_regexes, "series folder")
    validate_regex_patterns(settings.show_season_patterns.season_folder_regexes, "season folder")
...
def recognize_media_path(
    ...
    for index, part in enumerate(parts[:-1]):
        candidate_match = _match_first(settings.show_season_patterns.season_folder_regexes, part)
        if candidate_match:
            season_index = index
    ...
    series_match = _match_first(settings.show_season_patterns.series_folder_regexes, series_folder)
    if not series_match:
        return PathRecognition()

    series_title = (series_match.groupdict().get("title") or series_folder).strip()
    try:
